...
function MainApp({ user, workspace, onSwitchWorkspace, onLogout, onAuthExpired }) {
 ...
 const handleExportPDF = () => {
    ...
    const addFooter = () => {
      ...
      pdf.text(`Page ${page}`, pageWidth - margin - 15, pageHeight - 10);
    };
    ...
    const writeMetaRow = (label, value) => {
      ...
      pdf.text(String(value || "-").slice(0, 110), margin + 36, y);
      y += 5.6;
    };
    ...
    pdf.roundedRect(margin, 13, contentWidth, 34, 4, 4, "F");

    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(20);
    pdf.setTextColor(...palette.white);
    pdf.text("DevFlow", margin + 8, 26);

    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(9.4);
    pdf.setTextColor(203, 213, 225);
    pdf.text("AI-powered developer documentation", margin + 8, 37);
    ...
    lines.forEach((rawLine) => {
      ...
      const sourceFileMatch = trimmed.match(/^#?\s*Source File:\s*(.+)$/i);
      if (sourceFileMatch) {
        ensureSpace(18);
        y += 3;
        pdf.setFillColor(...palette.soft);
        pdf.roundedRect(margin, y - 5, contentWidth, 12, 2.5, 2.5, "F");
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(12.4);
        pdf.setTextColor(...palette.ink);
        pdf.text("Source File", margin + 4, y + 2.5);
        pdf.setFont("helvetica", "normal");
        pdf.setFontSize(10.2);
        pdf.setTextColor(...palette.slate);
        pdf.text(sourceFileMatch[1].slice(0, 90), margin + 36, y + 2.5);
        y += 12;
        return;
      }
      ...
      if (langMatch) {
        writeMetaRow("Language", langMatch[1]);
        return;
      }

      const totalMatch = trimmed.match(/^Total Lines:\s*(.+)$/i);
      if (totalMatch) {
        writeMetaRow("Total lines", totalMatch[1]);
        return;
      }

      const nonEmptyMatch = trimmed.match(/^Non-empty Lines:\s*(.+)$/i);
      if (nonEmptyMatch) {
        writeMetaRow("Non-empty lines", nonEmptyMatch[1]);
        y += 3;
        return;
      }
      ...
      if (/^#\s+/.test(trimmed)) {
        ...
        y += 3;
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(15.4);
        pdf.setTextColor(...palette.ink);
        pdf.text(heading.slice(0, 88), margin, y);
        y += 4;
        pdf.setDrawColor(...palette.blue);
        pdf.line(margin, y, pageWidth - margin, y);
        y += 5;
        return;
      }
      ...
      if (/^##\s+/.test(trimmed)) {
        writeSectionTitle(trimmed.replace(/^##\s+/, ""));
        return;
      }
      ...
      if (/^\|/.test(trimmed)) {
        ...
        if (cells.length >= 2) {
          writeMetaRow(cells[0], cells.slice(1).join(" | "));
          return;
        }
      }
